...
const createCheckbox = () => {
    ...
    $rows.forEach($row => {
        $row.querySelector('td:nth-child(5)').insertAdjacentHTML('beforeend', getCheckboxTemplate(uid++));
        $row.querySelector('td:nth-child(6)').insertAdjacentHTML('beforeend', getCheckboxTemplate(uid++));
    });
};
...
(() => {
    ...
    document.querySelector('.btn_inq').insertAdjacentHTML('beforeend', `
        <button type="button" class="ktx-macro-button">${isStarted ? '자동 예매 정지' : '자동 예매 시작'}</button>
    `);

    document.querySelector('.ktx-macro-button').addEventListener('click', isStarted ? macroStop : macroStart);
